...
{% set import_util_dates = unit_testing_select_table(ref('dim_dates'), ref('unit_test_input_util_dates')) %}
...
subscription_periods AS (
    SELECT
        ...
        ends_at,
        start_month,
        {{ subscription_ending_month_date_cleanup('start_month', 'end_month') }}
    FROM
